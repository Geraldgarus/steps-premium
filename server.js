...
// ── TEMPORARY DATABASE EXPORT ENDPOINT ──────────────────────────────────────
// Protected by secret key. REMOVE THIS ENDPOINT AFTER MIGRATION IS COMPLETE.
app.get('/api/admin/export-db', async (req, res) => {
  const secret = req.query.key;
  if (!secret || secret !== (process.env.EXPORT_SECRET || 'steps-export-2026')) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
  const tables = [
    'apartments', 'reservations', 'users', 'store_items',
    'store_requests', 'outlet_inventory', 'outlets',
  ...
  try {
    ...
    for (const table of tables) {
      try {
        ...
        if (rows.length === 0) { sql += `-- ${table}: empty\n\n`; continue; }
        ...
        sql += `-- ${table}: skipped (${e.message})\n\n`;
      }
    }
    res.setHeader('Content-Type', 'text/plain; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="steps_premium_export.sql"');
    res.send(sql);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
